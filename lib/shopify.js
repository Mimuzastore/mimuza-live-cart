...
export async function addReservation({username,code,title,color,size,price,quantity}){
  ...
  // Rebuild current custom line items and append the new one.
  // This app intentionally creates only custom line items for LIVE reservations.
  const current=(existing.lineItems?.nodes || []).map(li=>({
    title:li.title,
    quantity:li.quantity,
